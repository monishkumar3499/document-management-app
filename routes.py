...
from services import db_service, process_document, ocr_service
from models import Document, DepartmentContent, ExtractedText, DepartmentTable
...
# ------------------ Get Page-wise OCR Tables (RAW, per document) ------------------ #
@router.get("/api/v1/documents/{doc_id}/ocr-tables")
async def get_pagewise_ocr_tables(doc_id: str):
    """
    Raw tables as stored in ExtractedText.tables (no department filtering).
    Still useful for debugging or full-document inspection.
    """
    try:
...
# ------------------ NEW: Department-specific Tables (from DepartmentTable) ------------------ #
@router.get("/api/v1/departments/{doc_id}/{department}/tables")
async def get_department_tables(doc_id: str, department: str):
    """
    Return only the tables that were classified into this department,
    from the department_tables table.
    """
    try:
        doc_uuid = uuid.UUID(doc_id)
    except ValueError:
    ...
    async with db_service.SessionLocal() as session:
        ...
        # Fetch dept-specific tables
        result_tables = await session.execute(
            select(DepartmentTable)
            .where(
                and_(
                    DepartmentTable.document_id == doc_id,  # stored as String in model
                    DepartmentTable.department.ilike(department)
                )
            )
            .order_by(DepartmentTable.page_number.asc())
        )
        dept_tables = result_tables.scalars().all()

        if not dept_tables:
            # Not an error: just means no tables assigned for this dept
            return {
                "document_id": str(doc.id),
                "filename": doc.filename,
        ...
        pages_dict = {}
        for tbl in dept_tables:
            p = tbl.page_number
            if p not in pages_dict:
                pages_dict[p] = []
            # rows is already normalized 2D array
            pages_dict[p].append({
                "rows": tbl.rows,
        ...
        pages = [
            ...
            for page_num, tables in sorted(pages_dict.items(), key=lambda x: x[0])
        ]
...
async def list_all_documents():
    async with db_service.SessionLocal() as session:
        ...
        for doc in docs:
            ...
            dept_approvals = {
                d.department: getattr(d, "approved", False) for d in dept_contents
            }
            assigned_approvals = [
                approved for approved in dept_approvals.values() if approved is not None
            ]
...
# ------------------ Get Page-wise OCR Text with Tables (fresh OCR) ------------------ #
